...
const TodoList = ({ mode, date }: Props) => {
  ...
  const onChangeToggle = (mainTodoIndex: number) =>
    setToggleState((prevState) =>
      prevState.map((v, i) => (i === mainTodoIndex ? !v : v))
    );

  const isEmptyTodo = ({ mainTodo, subTodos }: TodoItem) =>
    mainTodo.content === "" && subTodos.length === 0;

  const onDragEnd: Responders["onDragEnd"] = (result) => {
    if (!result.destination) {
      return;
    }
    const sortedItems = dnd.reorder<TodoItem>(
      todos,
      result.source.index,
